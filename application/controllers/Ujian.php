...
class Ujian extends CI_Controller
{
	...
	public function save()
	{
		...
		$status_essay   = $this->input->post('status_essay', true);
		$status_curang  = $this->input->post('status_curang', true);
	...
	public function lanjutkanujian()
	{
		$id = $this->input->get('key', true);
		$key = urlencode($this->encryption->encrypt($id));
		redirect('ujian/?key=' . $key, 'refresh');
	}
	...
	public function index()
	{
		...
		$soal_essay = $this->ujian->getSoalEssay($id);
		...
		$q_soal = $h_ujian->row(); //ambil hasil ujian
		...
		if (!empty($soal_urut_ok)) {
			foreach ($soal_urut_ok as $s) {
				...
				$html .= '<div class="step oegh" id="widget_' . $no . '">';
			...
			if ($ujian->status_essay == "1") {
				if (!empty($soal_essay)) {
					// $no++;
					$path  = 'uploads/soalessay/';
					$html2 .= '<div class="step hallo" id="widget_' . $no . '">';
					$numb = 1;
					foreach ($soal_essay as $se) {
						$html2 .= '<div class="text-center"><div class="w-25">' . tampil_media($path . $se->file_essay) . '</div></div>' . $numb . '. ' . $se->essay_soal . '<br><br>';
						$numb++;
					}
					$html2 .= '</div></div>';
				}
		...
		$data = [
			...
			'html2'		=> $html2,
			'id_tes'	=> $id_tes,
			'status_essay' => $ujian->status_essay
		];
		$this->load->view('_templates/topnav/_header.php', $data);
	...
	public function simpan_akhir()
	{
		...
		//$nilai_bobot = $byobot;
		$nilai_bobot = ($total_bobot / $jumlah_soal);
		$nilai = $jumlah_benar * $nilai_bobot;
	...
	public function akumulasikan()
	{
		$id_tes = $this->input->get('ujianid');
		// Get Jawaban
		$list_jawaban = $this->ujian->getJawaban($id_tes); //mengambil jawaban dari h_ujian
